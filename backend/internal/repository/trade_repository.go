...
type TradeRepository interface {
	...
	GetTariffCodeByHSCode(ctx context.Context, companyID uuid.UUID, hsCode string) (*models.TariffCode, error)
...
func NewTradeRepository(db interface{}) TradeRepository {
	...
	if !ok {
		return NewTradeRepositoryImpl(nil)
	}
	return NewTradeRepositoryImpl(gormDB)
}